            ...
            <div class="row justify-content-end">
                <div class="col-lg-5 col-xl-4 col-xl-3 ml-sm-auto">
                    ...
                    @if (!$order->trashed())
                        ...
                        @if (in_array($order->status, [\App\Order::CREATED, \App\Order::CONFIRMED]))
                        ...
                        @if ($order->isDelivered())
                            ...
                            <form id="complete-form-<?=$order->id?>" action="{{ url('/admin/orders/complete/' . $order->id) }}" method="POST" style="display: none;">
                                @csrf
                            </form>
                        @endif

                        @if (!in_array($order->status, [\App\Order::DELIVERED, \App\Order::COMPLETED]))
                            <a href="#" class="btn btn-block mt-2 btn-lg btn-secondary btn-pill delete" order-id="{{ $order->id }}"> Remove</a>

                            <form id="delete-form-<?=$order->id?>" action="{{ url('/admin/orders/' . $order->id) }}" method="POST" class="delete" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        @endif
