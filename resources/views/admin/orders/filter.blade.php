<form action="{{ url(Request::path()) }}" method="GET" class="my-2">
    <div class="d-flex align-items-center justify-content-between">
        ...
        <div class="">
            <input type="text" name="start" class="form-control datepicker" placeholder="Start Date" value="{{ !empty(request()->input('start')) ? request()->input('start') : '' }}">
        </div>
        <div class="">
            <input type="text" name="end" class="form-control datepicker" readonly placeholder="End Date" value="{{ !empty(request()->input('end')) ? request()->input('end') : '' }}">
        </div>
